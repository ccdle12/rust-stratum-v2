use crate::{
    error::{Error, Result as NoiseResult},
    signature_noise_message::SignatureNoiseMessage,
    types::{AuthPubKey, StaticPublicKey},
};
use bitcoin::util::base58;
use std::{convert::TryInto, io};
use stratumv2_serde::{types::unix_timestamp::unix_u32_now, Serializable};
...
pub struct CertificateFormat<'a, T: AuthPubKey> {
    authority_public_key: T,
    static_public_key: &'a StaticPublicKey,
    signature_noise_message: &'a SignatureNoiseMessage,
}

impl<'a, T: AuthPubKey> CertificateFormat<'a, T> {
    pub fn new(
        authority_public_key: &'a str,
        static_public_key: &'a StaticPublicKey,
        signature_noise_message: &'a SignatureNoiseMessage,
    ) -> NoiseResult<CertificateFormat<'a, T>> {
        ...
        Ok(CertificateFormat {
            authority_public_key: T::from_bytes(&key_bytes)?,
            static_public_key,
    ...
    pub fn verify(&self) -> NoiseResult<()> {
        // TODO: This should be a noise error
...
// static public key.
impl<T: AuthPubKey> Serializable for CertificateFormat<'_, T> {
