...
impl CipherState {
    ...
    pub(crate) fn write_message_regular(&mut self, in_out: &mut [u8]) -> Result<(), NoiseError> {
        // TODO TMP: Splits the message into the plaintext to be encrypted and a mac
...
impl SymmetricState {
    ...
    pub fn initialize_symmetric(protocol_name: &[u8]) -> Self {
        ...
        match protocol_name.len() {
            ...
            32 => {
                println!("32");
                h = Hash::from_bytes(from_slice_hashlen(protocol_name));
            }
            _ => {
                println!("> 32");
                h = Hash::from_bytes(hash(protocol_name));
            }
        }
...
impl HandshakeState {
    ...
    pub(crate) fn initialize_initiator(prologue: &[u8], s: Keypair, psk: Psk) -> HandshakeState {
        let protocol_name = b"Noise_NX_25519_ChaChaPoly_BLAKE2s";
