...
impl Deserializable for Message {
    fn deserialize(parser: &mut ByteParser) -> Result<Message, stratumv2_serde::Error> {
        ...
        if message_type.channel_bit() != channel_bit {
            return Err(stratumv2_serde::Error::UnexpectedChannelBit(channel_bit));
        }
...
// pub fn frame<T: Frameable>(payload: &T) -> crate::error::Result<Message> {
pub fn frame<T: Frameable>(payload: &T) -> Result<Message, stratumv2_serde::Error> {
...
/// the Frameable trait.
pub fn unframe<T: Frameable>(message: &Message) -> Result<T, stratumv2_serde::Error> {
    let expected_message_type = T::message_type();
    if expected_message_type != message.message_type {
        return Err(stratumv2_serde::Error::UnexpectedMessageType(
...
#[cfg(test)]
mod tests {
    use super::{frame, unframe, Frameable, Message};
    ...
    impl TestMessage1 {
        fn new(x: u8) -> Result<TestMessage1, stratumv2_serde::Error> {
            Ok(TestMessage1 { x })
        }
    }
    ...
    impl TestMessage2 {
        fn new(x: u8) -> Result<TestMessage2, stratumv2_serde::Error> {
            Ok(TestMessage2 { x })
        }
    }
