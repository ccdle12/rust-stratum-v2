...
use stratumv2_job_negotiation as job_negotiation;
use stratumv2_mining as mining;
...
impl TryFrom<u8> for Protocol {
    ...
    fn try_from(byte: u8) -> Result<Self, stratumv2_serde::Error> {
        match byte {
            ...
            _ => Err(stratumv2_serde::Error::DeserializationError(
...
impl Deserializable for Protocol {
    ...
    fn deserialize(parser: &mut ByteParser) -> Result<Protocol, stratumv2_serde::Error> {
...
impl SetupConnection {
    ...
    ) -> Result<SetupConnection, stratumv2_serde::Error> {
...
impl Deserializable for SetupConnection {
    ...
    fn deserialize(parser: &mut ByteParser) -> Result<SetupConnection, stratumv2_serde::Error> {
        ...
        let variant = match protocol {
            ...
            _ => return Err(stratumv2_serde::Error::Unimplemented()),
...
macro_rules! impl_setup_connection_tests {
    ($protocol:expr, $fn:expr, $flags:ident) => {
        ...
        // use stratumv2_codec::codec::frame;
        use stratumv2_codec::frame;
        use stratumv2_serde::{deserialize, serialize, types::U24};
        ...
        fn test_serialize() {
            ...
            let result = serialize(&conn).unwrap();
            ...
            assert_eq!(result[5..9], serialize(&$flags::all()).unwrap());

            // Sanity check - deserializing back to the struct does not cause
            // errors.
            assert!(deserialize::<SetupConnection>(&result).is_ok());
        }

        #[test]
        fn test_serialize_empty_flags() {
        ...
        fn test_frame_message() {
            ...
            assert_eq!(
                deserialize::<U24>(&result[3..6]).unwrap(),
...
mod mining_setup_connection_tests {
    use super::*;
    use stratumv2_mining::SetupConnectionFlags;
...
mod job_negotiation_setup_connection_tests {
    use super::*;
    use stratumv2_job_negotiation::SetupConnectionFlags;
